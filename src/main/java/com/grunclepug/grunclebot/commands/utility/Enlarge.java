package com.grunclepug.grunclebot.commands.utility;
...
public class Enlarge extends ListenerAdapter
{
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event)
    {
        ...
        //Enlarge Command
        if(args[0].equalsIgnoreCase(Main.prefix + "enlarge"))
        {
            ...
            {
                event.getChannel().sendTyping().queue();
                event.getChannel().sendMessage("**invalid argument.** Usage: `" + Main.prefix + "enlarge {emote}` (custom emotes only)").queue();
            }
        }
    }
}
