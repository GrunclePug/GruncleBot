...
public class Pussy extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if(args[0].equalsIgnoreCase(Config.getPrefix() + "pussy") || args[0].equalsIgnoreCase(Config.getPrefix() + "coochie")) {
            String title = "bet you've never seen one of these before, virgin";
            int color = 0x8904B1;

            if(event.getTextChannel().isNSFW()) {
                try {
                    EmbedBuilder builder = new ImageAPI().getEmbed(NekoAPI.PUSSY, title, color);
