...
/**
 * Solo Girl Gif Command
 * @author GrunclePug
 */
public class SoloGirlGif extends ListenerAdapter {

    @Override
    public void onMessageReceived(MessageReceivedEvent event) {
        String[] args = event.getMessage().getContentRaw().split("\\s+");

        if(args[0].equalsIgnoreCase(Config.getPrefix() + "sologirlgif")) {
            String title = "every man's dream sight";
            int color = 0x8904B1;

            if(event.getTextChannel().isNSFW()) {
                try {
                    EmbedBuilder builder = new ImageAPI().getEmbed(NekoAPI.SOLO_GIRL_GIF, title, color);
