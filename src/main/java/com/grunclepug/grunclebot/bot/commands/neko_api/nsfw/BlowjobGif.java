 ...
 * @author GrunclePug
 */
public class BlowjobGif extends ListenerAdapter {

    @Override
    public void onMessageReceived(MessageReceivedEvent event) {
        String[] args = event.getMessage().getContentRaw().split("\\s+");

        if(args[0].equalsIgnoreCase(Config.getPrefix() + "blowjobgif") || args[0].equalsIgnoreCase(Config.getPrefix() + "bjgif")) {
            String title = "sluuuuurrrrpppp";
            int color = 0x8904B1;

            if(event.getTextChannel().isNSFW()) {
                try {
                    EmbedBuilder builder = new ImageAPI().getEmbed(NekoAPI.BLOWJOB_GIF, title, color);
