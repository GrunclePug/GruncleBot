 ...
 * @author GrunclePug
 */
public class FeetGif extends ListenerAdapter {

    @Override
    public void onMessageReceived(MessageReceivedEvent event) {
        String[] args = event.getMessage().getContentRaw().split("\\s+");

        if(args[0].equalsIgnoreCase(Config.getPrefix() + "feetgif")) {
            String title = "<:grab:949798072312021083> FEET";
            int color = 0x8904B1;

            if(event.getTextChannel().isNSFW()) {
                try {
                    EmbedBuilder builder = new ImageAPI().getEmbed(NekoAPI.FEET_GIF, title, color);
