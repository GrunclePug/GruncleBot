...
public class Feed extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if(args[0].equalsIgnoreCase(Config.getPrefix() + "feed")) {
            String title = "chomp chomp chomp";
            int color = 0x8904B1;

            if(args.length > 1) {
                Member target = event.getMessage().getMentionedMembers().get(0);
                title = target.getEffectiveName() + ", you got fed by " + event.getMember().getEffectiveName();
            }

            try {
                EmbedBuilder builder = new ImageAPI().getEmbed(NekoAPI.FEED, title, color);
