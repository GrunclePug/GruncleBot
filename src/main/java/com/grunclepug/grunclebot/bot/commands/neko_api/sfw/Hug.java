...
public class Hug extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if(args[0].equalsIgnoreCase(Config.getPrefix() + "hug")) {
            String title = "owo hugs";
            int color = 0x8904B1;

            if(args.length > 1) {
                Member target = event.getMessage().getMentionedMembers().get(0);
                title = target.getEffectiveName() + ", you got hugs from " + event.getMember().getEffectiveName();
            }

            try {
                EmbedBuilder builder = new ImageAPI().getEmbed(NekoAPI.HUG, title, color);
