...
public class PussyGif extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if(args[0].equalsIgnoreCase(Config.getPrefix() + "pussygif") || args[0].equalsIgnoreCase(Config.getPrefix() + "coochiegif")) {
            String title = "damnnnn that's pretty hot";
            int color = 0x8904B1;

            if(event.getTextChannel().isNSFW()) {
                try {
                    EmbedBuilder builder = new ImageAPI().getEmbed(NekoAPI.PUSSY_GIF, title, color);
