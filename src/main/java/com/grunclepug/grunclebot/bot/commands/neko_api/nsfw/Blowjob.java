...
public class Blowjob extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if(args[0].equalsIgnoreCase(Config.getPrefix() + "blowjob") || args[0].equalsIgnoreCase(Config.getPrefix() + "bj")) {
            String title = "slurp slurp gobble";
            int color = 0x8904B1;

            if(event.getTextChannel().isNSFW()) {
                try {
                    EmbedBuilder builder = new ImageAPI().getEmbed(NekoAPI.BLOWJOB, title, color);
