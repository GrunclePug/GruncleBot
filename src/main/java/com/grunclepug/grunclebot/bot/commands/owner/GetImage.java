 ...
 * @author GrunclePug
 */
public class GetImage extends ListenerAdapter {
    @Override
    public void onMessageReceived(MessageReceivedEvent event) {
        String[] args = event.getMessage().getContentRaw().split("\\s+");

        if(args[0].equalsIgnoreCase(Config.getPrefix() + "getimage")) {
            // Verify Identity
            if(event.getMember().getId().equals("247916497803018242") || event.getMember().getId().equals("474703059981697025")) {
                ...
                Message message = channel.retrieveMessageById(args[2]).complete();

                if(message.getAttachments().size() > 0) {
                    for(int i = 0; i < message.getAttachments().size(); i++) {
                        EmbedBuilder builder = new EmbedBuilder();
                        builder.setTitle("Image")
                                .setImage(message.getAttachments().get(i).getUrl());
