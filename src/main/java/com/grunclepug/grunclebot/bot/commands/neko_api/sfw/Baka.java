 ...
 * @author GrunclePug
 */
public class Baka extends ListenerAdapter {
    @Override
    public void onMessageReceived(MessageReceivedEvent event) {
        String[] args = event.getMessage().getContentRaw().split("\\s+");

        if(args[0].equalsIgnoreCase(Config.getPrefix() + "baka")) {
            String title = "BAKA BAKA BAKA!";
            int color = 0x8904B1;

            try {
                EmbedBuilder builder = new ImageAPI().getEmbed(NekoAPI.BAKA, title, color);
