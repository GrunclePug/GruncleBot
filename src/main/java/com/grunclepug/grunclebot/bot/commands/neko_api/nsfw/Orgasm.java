 ...
 * @author GrunclePug
 */
public class Orgasm extends ListenerAdapter {

    @Override
    public void onMessageReceived(MessageReceivedEvent event) {
        String[] args = event.getMessage().getContentRaw().split("\\s+");

        if(args[0].equalsIgnoreCase(Config.getPrefix() + "orgasm")) {
            String title = "ONIICHAN~~~ I'M CUMMING!";
            int color = 0x8904B1;

            if(event.getTextChannel().isNSFW()) {
                try {
                    EmbedBuilder builder = new ImageAPI().getEmbed(NekoAPI.ORGASM, title, color);
